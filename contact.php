
<!-- The Contact Page. This page displays contact information of the website owner (or business running the site)
 and also allows for anyone to contact the owner by through a form where they input their name, email and message-->
<div class='row my-5 py-5'>
  
  <div class='col-12 text-center'>
    <img src='img/log.png' height="auto"width="300px">
  </div>
  <div class='col-12 text-center'>
    <h2>Contact Us</h2>
  </div>
  <div class='col-sm-12 col-md-8 col-lg-5 mx-auto text-center'>

    <!-- Contact info -->
    <p>Phone: +64 12 34 5678</p>
    <p>Email: person@example.eg</p>
    <p>Location: blah blah blah blah blah blah blah blah</p>
  </div>
</div>
